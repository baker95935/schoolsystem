...
<body class="background_color">
	...
	<div class="container" style="height: 100%;">
		<div class="row title_class">
			...
			<div id='nowclass' class="col-xs-8  title_font_middle " > 我的班级</div>
			<div class="col-xs-2"><a href="<?php echo U('managestukeynote0402',array('userid'=>$userid,'paper_name'=>$paper_name,'classid'=>$classid,'testid'=>$testid,'username'=>$username,'realname'=>$realname));?>" style="color: white;font-size: 15px;display: block;margin-top: 1px;"></a></div>
		</div>
		<div class="row parent_nav">
		<div class="col-xs-3" >
				<select onchange="selectinfo()" name="subjectid" id="subjectid" class="blackselect">
				 
				<?php if(is_array($subjectList)): foreach($subjectList as $key=>$item): ?><option value ="<?php echo ($item["id"]); ?>"  <?php if($item["id"] == $subjectid): ?>selected="selected"<?php endif; ?>  ><?php echo ($item["subjectmsg"]); ?></option><?php endforeach; endif; ?>
			</select>
		...
		<div class="row">
			<div id="contentdiv" class="col-xs-12" style="height: 600px;overflow-y: auto;">
				<ol id="list1" id="list1" style="list-style-type:none;font-size: 15px;margin-left: -30px;padding-top:10px;">
                  <!-- <?php if(is_array($stu_data)): $i = 0; $__LIST__ = $stu_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$this_stu_id): $mod = ($i % 2 );++$i;?><li>
							<div style="padding-left: 20px;padding-top: 4px;width: 330px;overflow-x:scroll;white-space: nowrap;">
                              <?php echo ++$m;?>.&nbsp;&nbsp;&nbsp;姓名：<a href="<?php echo U('managestu_test0403',array('userid'=>$userid,'username'=>$username,'realname'=>$realname,'stu_id'=>$this_stu_id['userid'],'stu_name'=>$this_stu_id['realname'],'subject_msg'=>$subjectdata['subjectmsg'],'subject_id'=>$subjectdata['id']));?>"><?php echo ($this_stu_id['realname']); ?></a>&nbsp;&nbsp;&nbsp;&nbsp;错误率：<?php echo ($this_stu_id['test_ave_ratio']); ?>&nbsp;&nbsp;&nbsp;&nbsp;群组：<?php echo ($this_stu_id['groupname']); ?>
							</div>
							<hr>
...
<script>
	$(function(){
		...
		managetestdata(userid,nowpage,pagelength,classinfo,group,subjectid);
	});
	...
	function selectinfo()
    {
		...
		var userid=$('#userid').val();
		if(classinfo) {
			$('#nowclass').html($("#class").find("option:selected").text()+'班级');
		} else {
			$('#nowclass').html('我的班级');
		}
		$('#list1').html('');
	...
	function managetestdata(userid,nowpage,pagelength,classinfo,group,subjectid)
	{
		$.ajax({
			url:"<?php echo U('phpmanagestuscore0401');?>",
			data:{userid:userid,nowpage:nowpage,pagelength:pagelength,classinfo:classinfo,group:group,subjectid:subjectid},
			datatype:'json',
			type:'post',
			success:function(re){
				...
				for (var i = 0; i < mylength; i++) {
                    ...
                    var ahtml='/index.php/Home/headteacher/managestu_test0403/userid/'+$('#userid').val()+'/username/'+$('#username').val()+'/realname/'+$('#realname').val()+'/stu_id/'+testlist[i]['userid']+'/stu_name/'+testlist[i]['realname']+'/subject_id/'+$('#subjectid').val()+'/subject_msg/'+$("#subjectid").find("option:selected").text();
					//var ahtml='/index.php/Home/headteacher/managestu_test0403/userid/'+userid+'/username/'+$('#username').val()+'/realname/'+testlist[i]['realname'];
					addhtmlmsg=addhtmlmsg+'<li><div style="padding-left: 20px;padding-top: 4px;width: 330px;overflow-x:scroll;white-space: nowrap;">'+j+'.&nbsp;&nbsp;&nbsp;姓名：<a href="'+ahtml+'">'+testlist[i]['realname']+'</a>&nbsp;&nbsp;&nbsp;&nbsp;错误率：'+testlist[i]['test_ave_ratio']+'&nbsp;&nbsp;&nbsp;&nbsp;群组：'+testlist[i]['groupname']+'</div><hr></li>';
