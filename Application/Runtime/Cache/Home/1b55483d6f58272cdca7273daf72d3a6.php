...
<div id="bpdf"  onclick="buttontestpdf()" style="display:none;position: absolute;width: 80px;height:80px;background-color: #1b6d85;border-radius: 40px;top: 530px;;left: 70%;color: #ffffff;font-size: 16px;padding-top: 28px;padding-left: 24px;z-index:2;">PDF↓</div>
	...
	<div class="container" style="height: 100%;">
		<div class="row title_class">
			...
			<div class="col-xs-2"><a href="<?php echo U('stu_keylist0401',array('userid'=>$userid,'username'=>$username,'realname'=>$realname));?>" style="color: white;font-size: 15px;display: block;margin-top: 1px;">Key</a></div>
		</div>
...
<script>
  ...
  function downloadpdf(kind)
  {
	...
	function delsub(){
		...
		$.ajax({
			url:"<?php echo U('mykey_phpmanagelistdelsub');?>",
			data:{testid:testid},
			datatype:'json',
			type:'post',
			success:function(re){
