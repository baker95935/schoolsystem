...
<div class="contain" style="width: 100%">
    ...
    <div class="row" style="min-height: 26px;border: 1px solid #eaf0f2;text-align: center;font-size: 12px;padding-top: 4px;padding-bottom: 4px;">
        ...
        <span>&nbsp;&nbsp;|&nbsp;&nbsp;</span>
        <span>编辑区域：<input name="edit" type="radio" value="down" checked="checked"  />自动向下（D）&nbsp;&nbsp;<input name="edit" type="radio" value="up"/>自动向上（U）&nbsp;&nbsp;<input id="stopmove" name="edit" type="radio" value="stop" />手动操作（Esc）&nbsp;&nbsp;</span>
        <span>&nbsp;&nbsp;|&nbsp;&nbsp;</span>
                    ...
                    <div id="answercontentid" style="height: 300px;width:465px;overflow-y: scroll;">
                        ...
                        <table border="1px;" style="width: 460px;height: 300px;background-color: #2b669a;color:white;">
                            ...
                            <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$testdata): $mod = ($i % 2 );++$i;?><tr id="tr<?php echo ($testdata["id"]); ?>" name="tr<?php echo ++$m;?>"  style="height: 45px;">
                                    ...
                                    <td ><span id="title<?php echo ($testdata["id"]); ?>"><?php echo ($testdata["title"]); ?></span></td><td><span id="kind<?php echo ($testdata["id"]); ?>"><?php echo ($testdata["kind"]); ?></span></td><td><span id="answer<?php echo ($testdata["id"]); ?>"><?php echo ($testdata["answerid"]); ?></span></td><td><a id="<?php echo ($testdata["srcid"]); ?>"  href="javascript:void(0)" onclick="test1(this.id)">查看</a></td><td><a id="<?php echo ($testdata["id"]); ?>" name="<?php echo ($testdata["srcid"]); ?>" testid="<?php echo ($testdata["srcid"]); ?>" href="javascript:void(0)" onclick="choosetitle(this.id)">选择</a></td>
...
<input id="nowtestid" type="text" value="0">
...
//选择数据操作
    ...
    //  var testsrcid=$('a[id=35]').attr('testid').val();
      var testsrcid=$('a[id=35]').testid.val();
      
     //   var testsrcid=$("#"+id).attr("testid");
       //var testsrcid=$('35').attr("name");
...
function answersub(){
    ...
    if(titleeditkind=='re'){
      ...
      //alert('x:'+x+',y:'+y+',width:'+width+',height:'+height+',src:'+src+',nowtestid:'+nowtestid+',nowanswerid:'+nowanswerid);
