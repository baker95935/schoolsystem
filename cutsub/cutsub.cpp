...
Mat  rotatesub(string address, double angle)  //∑µªÿ–˝◊™∫ÛµƒÕº∆¨¥Û–°£¨Ω«∂»
{
    ...
    Mat temp2 = Mat(int(2 * r_y), int(2 * r_x), src.type(), Scalar(255, 255, 255));  //œ»––∫Û¡–
    
    Mat imageROI = temp2(Rect(int(r_x - 0.5*src.cols), int(r_y - 0.5*src.rows), src.cols, src.rows));  //œ»¡–∫Û––
    addWeighted(imageROI, 0, src, 1, 0., imageROI);
    Point center = Point(temp2.cols / 2, temp2.rows / 2);   //œ»¡–∫Û––
...
string cutsub(int kind, double x, double y, double width, double height, string address, int degree, double standarddx, double standarddy)
{
    ...
    switch (kind)
    {
        ...
        case 4:
        {
            Mat  dst_1 = src(Rect(0, 0, src.cols, y));
            Mat  dst_2 = src(Rect(0, y, src.cols, src.rows-y));
            ...
            double y_rotation1 = y/ standarddy;
            double x_rotation2 = src.cols/ standarddx;
            double y_rotation2 = src.rows-y/ standarddy;
...
//    double   width = 0;
//    double   height = 0;
//    string  address = "/Users/fangzheng/Desktop/1232/m5b3592ba0a11a.jpg";
//    double ang = -3.0;
//    double standarddx = 420;
//    double standarddy = 610;
