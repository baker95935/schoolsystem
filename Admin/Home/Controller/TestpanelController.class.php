...
class TestpanelController extends Controller
{
    ...
    public function test_list01()
    {
        ...
        $count= $model->where('statusmsg='.$statusmsg)->count();// 查询满足要求的总记录数
        $page = new \Think\Page($count,12);// 实例化分页类 传入总记录数并且每页显示5条记录
        $nowPage = isset($_GET['p'])?$_GET['p']:1;
        $data = $model->where('statusmsg='.$statusmsg)->order('creat_time desc')->page($nowPage.','.$page->listRows)->select();
...
//        print_r($data);
       ...
       // echo $subjectid;
        ...
        $questiontype= $model3->where('subjectid='.$subjectid)->select();
